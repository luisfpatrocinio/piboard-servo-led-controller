...
// A PIBoard utiliza os pinos 7 e 8 para comunicar com a tela, necessitando do
// I2C por Software. A classe "F" aloca 1024 bytes de RAM para um buffer oculto,
// evitando a cintilação (flicker).
...
void DisplayController::showWelcomeMessage() {
  ...
  const char *title = "PIBoard ServoCtrl";
  int textWidth = display.getStrWidth(title);

  // A biblioteca U8g2 desenha usando a base da letra (baseline), então o Y deve
  // ser maior que 0.
  ...
  delay(2000); // É seguro usar delay() aqui pois é apenas na inicialização
               // (setup)
...
void DisplayController::showState(int state, int direction,
                                  const __FlashStringHelper *ledName) {
  display.clearBuffer();
  display.setFont(u8g2_font_ncenB08_tr);

  // -- 1. Header Centralizado --
  const char *headerText = "PIBoard ServoCtrl";
  ...
  display.setFont(
      u8g2_font_ncenB14_tr); // Aumenta a fonte para destacar a direção

  const char *dirStr = "";
  if (state == 1) {
    dirStr = "---"; // Se estiver em 90 graus, exibe a barrinha indicando parada
  } else {
    dirStr = (direction == 1) ? ">>>" : "<<<";
  }
